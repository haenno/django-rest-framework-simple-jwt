...
if [ $(git -C $DRFJWT_SCRIPT_DIR rev-list HEAD...origin/main --count) != 0 ]; then
    ...
    sudo docker-compose -f prod-docker-compose.yml --project-directory $DRFJWT_SCRIPT_DIR stop
    sudo docker-compose -f prod-docker-compose.yml --project-directory $DRFJWT_SCRIPT_DIR build
    sudo docker-compose -f prod-docker-compose.yml --project-directory $DRFJWT_SCRIPT_DIR up -d
